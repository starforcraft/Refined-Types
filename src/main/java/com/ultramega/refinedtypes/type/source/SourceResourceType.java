...
public enum SourceResourceType implements ResourceType {
    ...
    public static final MapCodec<SourceResource> MAP_CODEC = RecordCodecBuilder.mapCodec(ins -> ins.group(
        Types.CODEC.fieldOf("source").forGetter(SourceResource::type)
    ).apply(ins, SourceResource::new));
    public static final Codec<SourceResource> CODEC = MAP_CODEC.codec();
    public static final StreamCodec<RegistryFriendlyByteBuf, SourceResource> STREAM_CODEC = StreamCodec.composite(
        Types.STREAM_CODEC, SourceResource::type,
        SourceResource::new
    );
    ...
    private static final ResourceLocation SPRITE = createRefinedTypesIdentifier("source_resource_type");
