...
import com.ultramega.refinedtypes.grid.view.energy.EnergyGridResource;
...
public abstract class TypeGridResourceType implements GridResourceType {
    @Override
    public GridResource apply(final ResourceKey resource) {
        ...
        final Map<GridResourceAttributeKey, Supplier<Set<String>>> attributes = Map.of(
            ...
            GridResourceAttributeKeys.TAGS, Suppliers.memoize(() -> this.getTags(type)),
            GridResourceAttributeKeys.TOOLTIP, Suppliers.memoize(() -> Set.of(this.getTooltip(type)))
        );
...
//            case SoulResource(Type type) -> type;
    ...
    private Set<String> getTags(final Type type) {
        return Types.TYPE_REGISTRY.getResourceKey(type)
            .flatMap(Types.TYPE_REGISTRY::get)
            .stream()
            .flatMap(Holder::tags)
